...
export default function MapPage() {
  ...
  useEffect(() => {
    const fetchPartnerStores = async () => {
      if (!affiliation) {
        console.log("소속 단과대학 정보가 없어 데이터를 가져오지 않습니다.");
        return;
      }
      ...
      try {
        ...
        if (snapshot.exists()) {
          ...
          const partnerStores: PartnerStore[] = Array.isArray(rawData)
            ? rawData
          ...
          console.log(`${topCategory} 필터링 결과:`, filteredByCategory.length);
          ...
          const convertedStores = filteredByCategory.map((ps) => {
            const normalized = {
              ...
              openingTime: ps.opening_time || ps.openingTime,
              closingTime: ps.closing_time || ps.closingTime,
              breakStartTime: ps.break_start_time || ps.breakStartTime,
              breakEndTime: ps.break_end_time || ps.breakEndTime,
              lastOrder: ps.last_order || ps.lastOrder,
              lat: Number(ps.lat),
              lng: Number(ps.lng),
            };
          ...
          console.log("변환된 Store 데이터:", convertedStores);
        } else {
    ...
    fetchPartnerStores();
  }, [affiliation, currentLocation, setStores, topCategory]);
